...
async function checkOneVariant(
  ...
  try {
    const res = await fetch(CLAUDE_API_URL, {
      ...
      // Timeout guard matches the current agent standard (bugs #35/#41):
      // a slow response must fail clean, not hang the isolate until the
      // platform kills it. Diya predates this — added on restore.
