...
- **Deploy** is push, CI green, promote, then confirm the build stamp. `deploy-functions.yml` auto-deploys `supabase/functions/**` on `main`; `workflow_dispatch` verifies the token before you trust it. review-build deploys to TEST by hand (`supabase functions deploy --use-api`, `supabase db push`). The `cc.awaas.world` **client** build is Vercel-hosted and its promote is unreliable — it auto-deployed once (verified 2026-09-12) and then needed a manual promote in the Vercel dashboard on both 2026-09-18 and 2026-09-22, each confirmed only after a stamp mismatch. Never assume either way: push, watch CI, fetch the bundle and grep `__COMMIT_SHA__` — if it doesn't match HEAD, ask Saswat to promote in the dashboard (never `supabase link`/CLI, which stays unlinked), then re-check the stamp. Confirm what is live before claiming it — `gh run list --workflow=deploy-functions.yml`, or the live behaviour itself.
