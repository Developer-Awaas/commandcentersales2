...
**Canva** — per-user OAuth (`org_user_integrations`). `canva-open-editor` uploads asset, creates design, returns `{ editUrl, designId }`. `canva-sync-design` exports via `POST /v1/exports`, downloads PNG, overwrites the original storage path. `ImageGalleryViewer` shows "Sync from Canva" after opening the editor. `canva-oauth-callback` handles the OAuth redirect.
- **OAuth connect is a popup, not a same-tab redirect**: `src/lib/canva-oauth-popup.ts`'s `openCanvaOAuthPopup()` is the only place that opens Canva's authorize URL — `window.open(authUrl, 'canva-oauth', ...)`, never `window.location.href`. This is the *only* thing every cold-start call site (`CanvaConnectButton`, `ImageGalleryViewer`) goes through now. Completion is signalled back via `postMessage`: `CanvaReturn.tsx` checks `window.opener` — if present (popup case), it posts `{type: 'canva-oauth-complete', connected, error, creativeId}` to the opener and `window.close()`s itself instead of navigating; the opener's listener (registered by `openCanvaOAuthPopup`) then re-runs whatever action was pending (e.g. `handleCanva(img)` again, now succeeding with a real `editUrl`) — the original tab is never touched, no state is ever lost. Falls back to the old same-tab `window.location.href` redirect only if `window.open()` returns `null` (blocked) — that fallback path is why the DB-backed resume (bug #46) and `beforeunload` suppression still exist, not because the popup path needs them.
