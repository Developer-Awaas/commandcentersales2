...
| `aanya_training_creatives` | `20260613000000` | Real-world creatives Aanya trains on. `source`/`performance_tier` CHECKs. `vision_analysis jsonb`. Images in `brand-assets/aanya-training/{orgId}/` |
...
| 29 | `aanya_training_creatives` | Table created via API with wrong RLS → INSERT always blocked. **Fix**: migration `20260613000000` recreates org-scoped policies. |
...
- **CI gate**: `.github/workflows/typecheck.yml` — **five** parallel jobs on every push/PR to `main`: `client-unit-tests` (Vitest), `build` (`tsc --noEmit` strict + `vite build`), `edge-typecheck` (`deno check` on all 10 Edge Function entry points), `edge-unit-tests` (`deno test` on `_shared/agents/`), `ws1-6-isolation` (see below). Cannot be bypassed with `--no-verify`. Zero TypeScript errors — do not add new ones. New `supabase/functions/*/index.ts` → add to the `deno check` list here, `scripts/hooks/pre-push`, and `deploy-functions.yml`'s deploy loop.
  - **CI was silently broken for ~a month** (every run since commit `92e7b4f`, 2026-06-26, failed) — nobody was watching. Fixed 2026-07-21: real pre-existing TS errors (`DhruvInsightCards.tsx` bad import path + unused `React` import; `Strategy.tsx` missing `meta_ad_account_id` field) plus a genuinely flaky, Linux-CI-specific Deno/npm interop issue (`edge-unit-tests` intermittently can't resolve `npm:@types/node`, root cause not fully identified — reproduced only on Linux, never locally on Windows even with the exact CI Deno binary; `nodeModulesDir: "auto"`, explicit `deno.lock` pinning, explicit `--lock` flags, and an explicit `deno install` step are all in place as real, defensible mitigations, but this job can still intermittently fail — if it does, that's a known open issue, not a new regression to chase from scratch).
