...
- **Project media (`project_assets`) vision enrichment (fixed 2026-07-02)**: `buildSeniorDesignerCreativePrompt` auto-fetches `project_assets` whenever `project_id` is set, but a bare `asset_url` string in a text prompt is invisible to a text-to-image model — `ProjectAsset.visual_description` must be populated for the reference to have any real effect (the DB column doesn't exist, so this only ever happens client-side, per-call). `Creatives.tsx`'s 3-variant flow already did this correctly (fetches `project_assets`, runs `describeImageForFlux(url)` on `hero_exterior`/`interior_*`/`amenity_*` types, passes the enriched array in). **`Strategy.tsx`'s Quick Generate and Full Strategy (Aanya upgrade) paths were missing this step entirely** — they never passed a `project_assets` array at all, silently falling through to the auto-fetch-with-no-vision-description path, so a project's real uploaded hero photo had zero actual influence on generation despite existing in the DB. Both paths now run the same enrichment inline (mirrors `Creatives.tsx`) before calling `buildTwoStageQuickGenerateBrief`. If you add a fourth generation entry point, replicate this same fetch-and-enrich block — there is no shared helper for it yet (three near-identical inline copies as of this fix; extract one if a fourth appears).
...
- **Text-overlay layer system (added 2026-07-02)** — ad copy (headline/price/CTA) is now ALSO rendered as an app-controlled, editable overlay stored in `creative_assets.text_layers` (jsonb), independent of whatever text Section 6 bakes into the image. See "Text-Overlay Layer System" below. `ad-compositor.ts` was removed (superseded by `src/lib/text-layers.ts`). Section 6 itself is UNCHANGED in this pass — see that section for why.
...
- **Phase 5 (complete, confirmed live 2026-07-21)**: streaming delegation status via Realtime, approval gate, memory write. See below.

#### Phase 5 — Realtime turn tracking, approval gate, memory write (complete; confirmed deployed 2026-07-21)

**Timeline of a real deployment-pipeline incident (2026-07-21), kept here as the record**: while building WS1.6 isolation-harness probe 7, calling the live `aarav-orchestrate` with `action: 'approve'` didn't hit the approve dispatch at all — it ran a full normal `send_message` turn instead (real Arjun call, real cost). Root cause: `deploy-functions.yml` had exactly one run in its entire history, and it failed with `401 Unauthorized` from the Supabase Management API (`SUPABASE_ACCESS_TOKEN` invalid/expired) — at commit `92e7b4f`, the commit immediately before Phase 5 was added. That token had actually been rotated 18 minutes after the failure, but the workflow was never re-run to confirm the fix, so the deployed function silently stayed on pre-Phase-5 code for nearly a month. **Fixed 2026-07-21**: manually triggered `deploy-functions.yml` via `workflow_dispatch` (deploys whatever's on `origin/main`, no new code) — succeeded in 35s. Re-ran the full 9-probe isolation suite immediately after: **9/9 passed, including probe 7**, confirmed again in CI (run `29828405683`, job `88626996858`, `WS1.6 isolation harness`, green). `handleApprove`'s org-scoped `agent_turns` filter (index.ts line ~758-770) is now confirmed live, not just present in source.

**Lesson, added to this repo's definition of done**: a merged commit is not a deployed fact. "Phase N complete" claims in this file must be re-verified against what's actually live (`gh run list --workflow=deploy-functions.yml`, or the live behavior itself) before being trusted for anything security- or correctness-sensitive — merge-complete and deploy-complete are different claims and this file should not conflate them again.

**Separate, still-open finding from the same push (2026-07-21)**: `typecheck.yml` has been failing on every run since commit `92e7b4f` (2026-06-26) — confirmed identical `Client build` TypeScript errors (`DhruvInsightCards.tsx` unused `React` import + `Cannot find module '../lib/supabaseClient'`; `Strategy.tsx` a `Project` type mismatch) on both the pre-push-2026-07-21 run and the current one. This predates and is unrelated to the WS1.6/Phase-5 work — nobody had been watching `main`'s CI status for about a month. Not yet fixed; needs its own pass.
...
**2026-06-12**: Aanya's prompts now generate complete ad creatives with text rendered directly in the image (not as CSS overlays). This matches professional real estate advertising standards — headlines, pricing, CTAs, and feature boxes are all integrated into the image composition. Section 6 of the 9-section prompt specifies exact text content, fonts, sizes, colors, positions, and graphical containers (colored panels, buttons, etc.). GPT-Image-1 renders these text elements at generation time.

**2026-07-02 — known unreliability**: a controlled A/B test (same brief, same prompt, "₹ only, never $" instructions repeated in the system prompt, Stage 1, and Stage 2) still produced one image with a `$` price instead of `₹` — for both Sonnet- and Haiku-authored prompts. This is a GPT-Image-1 rendering-level failure, not a Claude prompt-writing failure — Claude never touches pixels, so no prompt engineering can fully guarantee compliance. This is why the app-controlled text-overlay layer system exists (see below) as a parallel, reliable path for critical text like pricing — Section 6 itself is intentionally left unchanged (see "Text-Overlay Layer System").

### Two-stage generation architecture (Quick Generate + Creatives 3-variant path)

Both `buildTwoStageQuickGenerateBrief` (Strategy Quick Generate) and `buildTwoStageVariantBriefs` (Creatives 3-variant, wraps the former per variant) in `senior-designer-prompts.ts` split what used to be one ~16000-token call into two stages to dodge the `claude-proxy` 120s timeout: **Stage 1** (one call, concept + ad copy + a `visual_anchor` field, `max_tokens: 1500`) → **Stage 2** (3 parallel calls, one per layout — main/portrait/story — each `max_tokens` 700-800, generating just that layout's 9-section image prompt). Total wall-clock ≈35-40s regardless of layout count, vs. the old single call's ~65s.

**`visual_anchor` (added 2026-07-02)** — Stage 2's 3 calls are independent, so each was free to invent its own building/scene, causing visibly inconsistent architecture across the 3 images in a set (confirmed in the same A/B test above). Stage 1 now also outputs `visual_anchor`: a literal 60-100 word physical description of one building, which every Stage 2 call is instructed to reproduce verbatim in its Section 1 scene narrative. This single change fixed cross-image consistency in testing — all 3 layouts showed the same building, same signage, same landscaping. Root cause was the two-stage split losing the single-completion's implicit shared context, not model choice (Sonnet vs. Haiku made no measurable quality difference in the same test — see git history/session notes for the full comparison).
...
- The text-overlay layer system (`text-layers.ts`, `TextLayerOverlay`, `creative_assets.text_layers`) — keep regardless; it layers on top of any image, independent of which model generated the background

---

## Text-Overlay Layer System

> Added 2026-07-02. See `.claude`-tracked plan history for the full design rationale (Canva API investigation, phasing decision).

**Why**: Section 6 baking text into the image is unreliable at the pixel-rendering level (see A/B test note above) — no amount of Claude-side prompt engineering fixes it, since Claude never touches pixels. This system gives the app a reliable, editable, app-controlled alternative for the same text.

**Scope decision (Phase 1 of 2)**: this pass is **additive only** — the AI image still renders baked text as before (Section 6 unchanged). The overlay is a second, independent layer on top. A separate future pass may simplify Section 6 to "reserve negative space" composition guidance and stop baking text at all — that carries real image-style risk (the Neelachala-style frame layout, e.g. `PRICE_BADGE`/`FEATURE_CHECKLIST`/`FOOTER_STRIP`, is partly built around baked typography) and needs its own A/B validation before being attempted.

**Why not Canva's text elements directly**: investigated and ruled out. Canva's Connect API `POST /v1/designs` only accepts `asset_id` — the uploaded image becomes an opaque background asset, with no way to seed separate text elements without a pre-built Brand Template + Autofill API (not implemented here). `canva-sync-design`'s export is always a flattened raster PNG too. So the editor is native to this app, not a Canva pass-through.

**Data model**: `creative_assets.text_layers jsonb` (migration `20260702000000`) — an array of `TextLayer` objects (`src/lib/text-layers.ts`): `{ id, text, xPct, yPct, widthPct?, fontSizePx, fontWeight, color, align, backgroundColor?, paddingPx?, borderRadiusPx? }`. All numeric fields are authored against a `TEXT_LAYER_REFERENCE_WIDTH` of 1080px and scaled to actual display/export width at render time, so a layer looks proportionally correct everywhere.

**Files**:
- `src/lib/text-layers.ts` — `TextLayer` type, `buildDefaultLayers(layout, adCopy)` (seeds 2-3 sensible default layers from existing ad copy), `renderTextLayers(imageSrc, layers, w, h)` (canvas compositor → flat JPEG data URL, for Download). Supersedes the old, unused `ad-compositor.ts` (deleted — zero call sites had ever wired it up).
- `src/hooks/useMeasuredWidth.ts` — shared `ResizeObserver` hook so the CSS preview and the interactive editor scale layers identically off the container's live rendered width.
- `src/components/TextLayerOverlay.tsx` — read-only live preview, `position:absolute inset-0` over an `<img>`, used by both `ImageGalleryViewer` and `CreativeViewer` (grid cards + lightboxes).
- `src/components/TextLayerEditor.tsx` — modal: drag-to-reposition (pointer events), click-to-select toolbar (text/font-size/color/bold/align/badge-background/delete), "+ Add Text", Save persists to `creative_assets.text_layers`.

**Seeding**: `StrategyResult.tsx`'s `handleGenerateWithGemini` and `Creatives.tsx`'s 3-variant upload loop both call `buildDefaultLayers` right after `uploadGeminiImageToSupabase` returns an `id`, using the same `ad_copy` fields already used elsewhere, then fire-and-forget `.update({ text_layers })` (non-fatal on error, matching the existing pattern around that upload call). Every newly generated creative starts pre-populated and immediately editable.

**Download**: both viewers' Download buttons check `textLayers?.length` — if present, bake via `renderTextLayers` before downloading (dimensions parsed from the `"Feed (1080×1080)"`-style label in `ImageGalleryViewer`, hardcoded 1080×1080 in `CreativeViewer` since that viewer's images are always square); otherwise fall back to the plain raw-image download.
...
| `creative_assets` | `20260604120000` | Generated images + editing lifecycle. Has `session_id uuid` to group 3-image sets. `text_layers jsonb` (migration `20260702000000`) — array of `TextLayer` objects, see Text-Overlay Layer System |
...
| `agent_memory_chunks` | `20260625120000` | pgvector memory layer for semantic + hybrid search. Columns: `id`, `org_id`, `project_id`, `scope memory_scope`, `agent_name`, `content`, `embedding vector(1024)` (nullable = fail-soft), `salience real CHECK 0..1`, `access_count`, `last_accessed_at`, `expires_at`, `source_memory_id → agent_memory(id)`, `created_at`. Enum `memory_scope`: `('decision','project','builder','domain','shared','agent')`. Indexes: HNSW cosine, GIN tsvector, three btree. RLS: org-scoped same pattern as all tables. RPCs: `match_memory_chunks` (hybrid scorer) + `touch_memory_chunks` (access counter). **Phase B1+B2 complete**: `_shared/embedding-provider.ts` (OpenAI `text-embedding-3-small` 1024-dim, Voyage stub, fail-on-dimension-mismatch, one retry) + `_shared/agent-memory.ts` (`writeMemory` fail-soft, `retrieveMemory` with fire-and-forget touch, `projectApprovedCampaign` wired into `handleApprove` fire-and-forget). Tenancy invariant: `projectApprovedCampaign(supabase, sourceMemoryId)` accepts **no org_id/project_id parameters** — it derives tenancy by SELECT-ing the source `agent_memory` row. Service-role bypasses RLS on writes; the SELECT is the verification step (one source of truth, no spoofable parameter hop). DOWN migration: `supabase/rollbacks/20260625120000_pgvector_memory_layer_down.sql`. |
...
| `CreativeViewer` | 3-col grid, skeleton loaders, Realtime, full action set (approve/reject/regen/canva/adobe/**edit text**/download), lightbox. Uses `creative-assets` bucket. Renders `TextLayerOverlay` on every card/lightbox image; Download bakes `text_layers` via `renderTextLayers` when present. |
| `ImageGalleryViewer` | Post-generation gallery (`GalleryImage[]`). Maintains `localImages` state so edits update in place. Shows "Sync from Canva" button after Canva opens. Passes `storagePath`+`storageBucket="brand-assets"` to `AdobeExpressModal`. Renders `TextLayerOverlay` on every card/lightbox image; "Edit Text" button opens `TextLayerEditor`. Download bakes `text_layers` via `renderTextLayers` when present, else falls back to a raw download. |
| `TextLayerOverlay` / `TextLayerEditor` | `src/components/TextLayerOverlay.tsx` (read-only, shared preview) / `TextLayerEditor.tsx` (drag-to-reposition modal). See Text-Overlay Layer System above. |
...
| 32 | `useAuth.ts` | Recurring "Session expired" during normal use every ~50-60 min. Root cause: `fetchOrCreateProfile()`'s outer `catch` called `signOut()` on ANY thrown error, including transient failures during Supabase's silent background `TOKEN_REFRESHED` event. | `fetchOrCreateProfile` now isolates the SELECT in its own try/catch that returns early (no sign-out) on transient failure. `signOut()` only fires for the two confirmed "no org_id" unprovisioned-account branches. |
| 33 | `src/lib/supabase.ts` `invokeEdgeFn` | "Generation failed — Session expired" on image/AI generation even with a valid session. Root cause: `getSession()` returns the cached session object even when the JWT `exp` claim inside it is past. `invokeEdgeFn` only refreshed when `session` was null — not when the token itself was expired. Result: expired JWT sent to the `generate-image` / `claude-proxy` edge functions, platform gateway returns 401, which surfaces as the session-expired error. | `invokeEdgeFn` now: (1) decodes the JWT `exp` claim via `jwtExpiredOrExpiringSoon()` and proactively calls `refreshSession()` if the token is expired or expiring within 30 s, BEFORE making the call; (2) on any auth-class error after the call, tries one forced refresh and retries if a genuinely new token was obtained (belt-and-suspenders for clock skew / race). **Critical sub-bug**: original `jwtExpiredOrExpiringSoon` used raw `atob()` on the JWT payload which ALWAYS threw (JWTs use base64URL encoding with `-`/`_` instead of `+`/`/`, no padding) making the function return `false` on every call — proactive refresh never fired. Fixed: convert base64URL → standard base64 (`replace(/-/g, '+').replace(/_/g, '/')` + padding) before `atob()`. |
| 34 | `src/pages/strategy/StrategyResult.tsx` `handleGenerateWithGemini` | "Failed to send a request to the Edge Function" on image generation. Root cause: React 18 Strict Mode in development fires `useEffect(() => { handleGenerateWithGemini() }, [])` twice on the same component instance (mount → cleanup → remount simulation), launching 6 concurrent `invokeEdgeFn` calls. Also `FunctionsFetchError` (network-level `fetch()` throw, e.g. transient blip) was not retried — only auth-class HTTP errors were retried. | (1) `generatingRef.current` guard in `handleGenerateWithGemini` — early-return if already in progress; reset in `finally` so manual Regenerate still works. (2) `invokeEdgeFn` now retries once on `FunctionsFetchError` ("Failed to send a request") with the same token — covers transient network blips without masking persistent failures. Regression tests in `src/lib/supabase.test.ts`. |
| 35 | `supabase/functions/claude-proxy/index.ts` | "Function failed due to not enough compute resources" (HTTP 546) on AI generation calls. Root cause: `fetch(ANTHROPIC_API_URL, ...)` had no timeout — if Anthropic API was slow, the Deno isolate hung until Supabase's wall-clock limit killed it. Supabase reports a killed/hung isolate as "not enough compute resources" rather than a timeout, masking the real cause. | Added `signal: AbortSignal.timeout(120_000)` to the Anthropic fetch (Supabase free-tier limit is 150 s — 120 s gives a clean timeout before the platform kills the isolate). On timeout, returns `errorResponse('Anthropic API timed out after 120 s...')` (HTTP 200, Anthropic-error-format) so callers see a clear message. |
| 36 | `src/pages/Strategy.tsx`, `Creatives.tsx` | "Anthropic API timed out after 120 s" on strategy/creative generation. Root cause: `claude-sonnet-4-6` generating 3 image prompts (500-800 words each) + ad copy = ~3500 output tokens takes >120 s at low server-load throughput (~30 t/s). **Historical note**: an earlier fix attempt switched generation calls to `claude-haiku-4-5-20251001` via a `modelOverride` param — this did NOT survive into the codebase (verified 2026-07-02: neither the committed `HEAD` nor any later working-tree state has a Haiku override anywhere in `Strategy.tsx`/`Creatives.tsx`); it was superseded before being committed. | The fix that actually shipped and is live today: split the single ~16000-token call into the **two-stage architecture** (`buildTwoStageQuickGenerateBrief`/`buildTwoStageVariantBriefs` in `senior-designer-prompts.ts`) — Stage 1 (concept+copy, `max_tokens: 1500`) then Stage 2 (3 parallel per-layout calls, `max_tokens` 700-800 each), both still on default Sonnet. Total wall-clock ≈35-40s, well under the timeout, with no model swap needed. See "Two-stage generation architecture" above. |
| 37 | `src/lib/senior-designer-prompts.ts` `buildTwoStageQuickGenerateBrief` | Three images from one generation showed visibly different buildings/architecture (confirmed 2026-07-02 A/B test) — e.g. one image a modern tower, another a British brick townhouse. Root cause: Stage 2's 3 calls (main/portrait/story) run independently in `Promise.all`, and only `ad_copy`/`design_dna_tags` were locked from Stage 1 — nothing constrained the actual building description, so each call was free to invent its own. Not a model-choice issue — reproduced identically with both Sonnet and Haiku. | Stage 1 now also outputs `visual_anchor` — a literal 60-100 word physical description of one building. Stage 2's locked-direction block instructs every layout to reproduce that description verbatim in its Section 1 scene narrative. Fixed cross-image consistency in testing (same building, signage, and landscaping across all 3 layouts). |
| 38 | `senior-designer-prompts.ts` Section 6 / GPT-Image-1 | Ad price rendered as `$` instead of `₹` despite "₹ only, never $/USD" being stated in the shared system prompt, the Stage 1 critical-check line, AND the Stage 2 critical-check line (three separate reinforcements). Reproduced with both Sonnet- and Haiku-authored prompts, each in a different layout slot. This is a GPT-Image-1 rendering-level failure — the text instruction was correct, the image model just didn't follow it. | No prompt-side fix exists for this (Claude cannot control pixel rendering). Motivated building the app-controlled text-overlay layer system (`creative_assets.text_layers`, see "Text-Overlay Layer System") as a reliable parallel path for critical text — Section 6 itself is intentionally left as-is pending a separate, validated follow-up. |
...
- **`match_memory_chunks` canonical signature** (B1+B2 complete — write path wired via `projectApprovedCampaign`, read path available via `retrieveMemory` for future Arjun context injection):
  ```
  match_memory_chunks(
...
- **CI gate (real enforcement)**: `.github/workflows/typecheck.yml` is the authoritative gate — **four** parallel jobs on every push and PR to `main`: (1) `client-unit-tests` → `npm test` (Vitest, `src/**/*.test.ts`); (2) `build` → `npm run typecheck` + `npm run build` (Node 20); (3) `edge-typecheck` → `deno check` on all 10 Edge Function entry points; (4) `edge-unit-tests` → `deno test --allow-env supabase/functions/_shared/agents/`. This workflow cannot be bypassed with `--no-verify`. `npm run typecheck` runs `tsc --noEmit` (strict mode) — codebase is at zero TypeScript errors; do not add new errors. When a new `supabase/functions/*/index.ts` is created, add it to the `deno check` list in both `typecheck.yml` and `scripts/hooks/pre-push`, AND add it to the `deploy-functions.yml` loop (with `--no-verify-jwt` only if the function is called without a user JWT, e.g. by pg_cron).
- **RLS isolation harness (WS1.6, added 2026-07-21)**: `supabase/tests/isolation/` — cross-tenant probes against `agent_memory_chunks` + `profiles`, modeled on the sibling `awaas-suite` repo's Gate P (same `lib.ts` probe-function shapes: `crossOrgReadDenied`/`crossOrgWriteDenied`/`crossOrgUpdateDenied`/`crossOrgDeleteDenied`, separate implementation — no shared package between the two repos). 6 probes: cross-org read/write/update/delete on `agent_memory_chunks`, cross-org read on `profiles`, and a self-escalation probe against `prevent_self_privilege_escalation()` (the Command-Center equivalent of Gate P's RBAC-RPC probes — this schema has no `assign_role`/`provision_org` to test instead). **Runs against the only Supabase project this repo has (`CommandCentre_Prod`) — there is no separate TEST project**; decided 2026-07-21 to proceed carefully rather than stand up a second project: every seeded row is identifiably prefixed (`isolation-probe-org-a`/`-b`), `seed-isolation-probes.sh` is idempotent, `cleanup-isolation-probes.sh` removes everything it creates. Org creation goes through `SUPABASE_SERVICE_ROLE_KEY` directly (organizations has no INSERT policy for `authenticated` at all — verified via migration `20260610150000`, so service-role-only org creation was already the schema's reality, not something this harness introduced); `profiles.org_id` assignment is a service-role UPDATE (bypasses `prevent_self_privilege_escalation()` since `auth.uid()` is null under service-role). **First real run 2026-07-21: 6/6 probes passed** (`deno test`, `ok | 6 passed | 0 failed`) against live seeded data, then cleaned up via `cleanup-isolation-probes.sh` — no probe data left in prod. That first run also surfaced a real bug, since fixed: `curl -d "$var"` on Windows Git Bash silently corrupts multi-byte UTF-8 (an em-dash in the seed content) passed via argv, which PostgREST then rejects as `PGRST102 "Empty or invalid json"` — looks like a JSON-formatting bug but isn't; switching `jq -n` → `jq -nc` (compact output) did NOT fix it. The actual fix: every request body in `seed-isolation-probes.sh` is now written to a temp file via `printf` and sent with `curl --data-binary @file`, never inline as a command-line argument — platform/locale-independent.

**Probes 7-8 added same day (§5.1 Deviation Register item)** — probes 1-6 only prove RLS holds for the user-JWT path through PostgREST; they say nothing about the service-role write path in `handleApprove`/`projectApprovedCampaign` (`aarav-orchestrate`), which bypasses RLS by design. Probe 8 (`match_memory_chunks` RPC never returns org B content to org A, any query) **passed live**. Probe 8b (static grep across every migration's `match_memory_chunks` definition, failing if any ever specifies `SECURITY DEFINER`) **passed** — genuinely offline, reads `supabase/migrations/*.sql` directly, not a runtime call. Probe 7 initially failed — not a leak, but the discovery that `aarav-orchestrate` wasn't actually deployed with Phase 5 code at all (see Phase 5 section above for the full incident). Diagnosing this accidentally triggered two real Arjun strategy calls against the probe org (real LLM cost) — cleaned up from `agent_interactions` afterward.

**§5.1 CLOSED 2026-07-21.** After the deploy-pipeline fix (Phase 5 section above), re-ran the full suite fresh: **9/9 probes passed** (`deno test`, `ok | 9 passed | 0 failed`), including probe 7 against the now-correctly-deployed function. Confirmed a second time in CI: run [`29828405683`](https://github.com/Developer-Awaas/commandcentersales2/actions/runs/29828405683), job `WS1.6 isolation harness (agent_memory_chunks)` (`88626996858`), green — this is the citable artifact for the Deviation Register. `ws1-6-isolation` is wired into `typecheck.yml` with job-level secrets (7 repo secrets provisioned 2026-07-21: `REST_BASE`, `ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `PROBE_ORG_A/B_EMAIL/PASSWORD`) so every future push re-probes automatically rather than relying on memory of a past manual run.

**Still deliberately not wired into `typecheck.yml`** — every run so far has been manual, local, with credentials from `supabase/tests/isolation/.env.isolation.local` (gitignored, not the repo's actual GitHub Actions secrets yet — confirmed via `gh secret list`, only `SUPABASE_ACCESS_TOKEN` exists there today). Wiring a CI job needs those provisioned as real repo secrets first, on the isolation job only (job-level `env:`, never workflow-level, so `typecheck`/`build` never see the service-role key). Even once wired, probe 7 will correctly stay red until `aarav-orchestrate` is redeployed — that's accurate signal, not a broken test. Only 2 of the schema's 36 RLS-enabled tables are probed for the read/write/update/delete shape so far; `supabase/tests/isolation/README.md` documents what's covered and what a next extension needs (join-scoped tables, user-scoped-not-org-scoped tables, tables with no legitimate seed path yet).
- **Client unit tests (Vitest)**: `npm test` runs `vitest run` (single-pass). `npm run test:watch` for dev mode. Config: `vitest.config.ts` (jsdom environment). Setup: `src/test/setup.ts`. Test files colocated with source as `*.test.ts` under `src/`. Use explicit imports from `'vitest'` (no globals). Mock supabase and other external modules using `vi.hoisted` + `vi.mock` at module boundary — do NOT mock individual supabase query methods at call-site level (too brittle). Test files: `src/hooks/useAuth.test.ts` (3 tests — signOut catch-block contract, bug #32); `src/lib/supabase.test.ts` (10 tests — base64URL JWT decode, FetchError retry, auth retry, 546-crash no-retry, bug #33/#34); `src/lib/text-layers.test.ts` (6 tests — `buildDefaultLayers` field mapping, layout-specific positioning, coordinate bounds).
- **Catch-block discipline** (rule added 2026-07-01 to prevent recurrence of bug #32): Any `catch` block that takes a **destructive side effect** (signOut, navigate, delete, state wipe, DB write) MUST have a paired unit test proving the side effect does NOT fire on a transient/unexpected error. A generic `catch { destructiveAction() }` is always wrong — the destructive action must only fire when the specific failure case is positively confirmed (e.g. `if (!org_id)` after a successful SELECT, never inside the SELECT's own catch). Any background-recurring function (runs on `onAuthStateChange`, `setInterval`, Realtime subscription) containing a catch with a side effect is the highest-risk pattern and needs the test most urgently. Use `/code-review` to audit diffs for this pattern before finishing any auth-adjacent task.
...
- **Local pre-push hook (optional, committed)**: `scripts/hooks/pre-push` mirrors the CI checks and IS committed to the repo. Opt in once per clone: `git config core.hooksPath scripts/hooks`. Runs: `npm test` → `npm run typecheck` → `npm run build` → `deno check` (skipped if deno not on PATH) → `deno test`. Bypass with `git push --no-verify` only in a genuine emergency — CI remains the real backstop. `.git/hooks/pre-push` (untracked, if present from a prior session) is superseded by this committed version.
