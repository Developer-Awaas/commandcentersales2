...
function SeniorDesignerResultPanel({ data, languages, onRetry, savedId, project, projectId, funnelStage, onGeminiStateChange, resumedGalleryImages, heroImages, replicateAspect, textOverlayMode, zones }: {
  ...
  // mount/cleanup/mount, which would otherwise fire the auto-generate call
  // (3 real GPT-Image-1 requests) twice per Quick Generate result. Also
  // skips auto-generation entirely when resuming with images that already
  // exist — regenerating them would silently double real GPT-Image-1 spend.
  const hasAutoGeneratedRef = useRef(!!resumedGalleryImages?.length);
  // Set right before a Canva cold-start OAuth connect navigates away —
  // that round-trip is fully recoverable now (Strategy.tsx's resume effect
  // reloads everything from the DB), so the native "Leave site?" prompt
  // below would only ever get in the way of the very first Edit-in-Canva
  // click right after generation (before Save), not protect real data.
  ...
  useEffect(() => {
    ...
    // Replicate mode has no nanobanana_prompt_main (Stage 2 skipped) — trigger on
    // replicateAspect instead, or the edit would never fire.
    if (data.nanobanana_prompt_main || replicateAspect) {
  ...
  async function handleGenerateWithGemini() {
    ...
    try {
      ...
      // — a SHORT edit directive built from ad_copy ONLY (Stage 2 / 9-section was
      // never run; see the layouts:[] hard branch in Strategy.tsx). The normal
      // path keeps the 3-up feed/portrait/story batch.
      const primaryLang = (languages[0] ?? 'English').toLowerCase();
      const replicateCopy = {
        headline: data.ad_copy?.[`headline_${primaryLang}`] ?? data.ad_copy?.headline,
      ...
      // Evidence log (STEP 1): the assembled replicate directive must contain NO
      // 9-section content (no scene narrative / composition% / camera / lighting
      // / palette / tech-specs). Verify in the browser console during TEST eval.
      if (replicateAspect) {
        console.log('🎨 [REPLICATE PROMPT] final directive (must contain NO 9-section content):\n' + slots[0][3]);
      }
