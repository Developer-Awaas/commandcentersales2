...
export function buildReplicateLayoutPrompt(): string {
  return [
    ...
    "IMAGE 2 is the REAL PROJECT BUILDING (hero) — the ONLY building/subject. Render THIS building inside image 1's photo area(s). Do NOT keep, copy, or reference the building shown in image 1; image 1 is layout only.",
    // RB-P4 (iteration 3): flat colour swatches read as "blank rectangles" and
    // looked unfinished. Switch to SEAMLESS BACKGROUND CONTINUATION — the design's
    // own backdrop flows through former text areas so the template looks like an
    // intentional, clean, text-free composition rather than a form with empty boxes.
    'CRITICAL — THIS IS A BLANK, TEXT-FREE TEMPLATE, NOT A FINISHED AD. Render ZERO readable content: no glyphs, letters, numbers, digits, words, punctuation or symbols in ANY language or script (Latin, Devanagari, Arabic, CJK — none), and no faint, embossed, placeholder "Lorem", watermark, partial or blurred lettering. Wherever the reference has a text-bearing zone (headline bar, subheadline, price callout, every badge/stat cell, the checklist panel, the CTA button, the footer/contact strip), DO NOT paint a flat colour block, band, bar, box, plate or any placeholder shape there. Instead let the surrounding design continue SEAMLESSLY through that area — the sky, wall, façade, landscaping, gradient or base colour of that region flows through uninterrupted, as if the layout were simply designed with generous text-free negative space. All real text is added later by software.',
    'Keep the reference\'s PHOTO/image areas, proportions and overall composition, but treat every former text zone as OPEN BACKGROUND: no visible zone rectangle, divider, bar or coloured plate may remain where text used to sit — the backdrop reads as one clean, continuous, intentional surface.',
