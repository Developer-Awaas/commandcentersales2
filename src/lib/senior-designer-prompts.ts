...
export function buildReplicateLayoutPrompt(): string {
  return [
    ...
    "IMAGE 2 is the REAL PROJECT BUILDING (hero) — the ONLY building/subject. Render THIS building inside image 1's photo area(s). Do NOT keep, copy, or reference the building shown in image 1; image 1 is layout only.",
    // FIX 1 (iteration 2): the earlier "leave zone empty / no letters" wording still
    // let ghost text bake. Attack it at full strength — the output is a TEMPLATE for
    // software, not a finished ad. Demand solid colour blocks, name every text form to
    // suppress (any language/script), and forbid the specific ghost failure modes.
    'CRITICAL — THIS IS A BLANK TEMPLATE, NOT A FINISHED AD. Every text-bearing zone (headline bar, subheadline, price callout, every badge/stat cell, the checklist panel, the CTA button, and the footer/contact strip) MUST be rendered as a SOLID, FLAT, EDGE-TO-EDGE block or band of its design colour — like an unlabelled swatch. It must be COMPLETELY DEVOID of any glyphs, characters, letters, numbers, digits, words, punctuation, symbols, logos or lettering OF ANY LANGUAGE OR SCRIPT (Latin, Devanagari, Arabic, CJK — none). No faint text, no embossed text, no placeholder "Lorem", no watermark text, no partial or blurred letters. If you feel an urge to write anything inside a zone, fill it with flat colour instead. All real text is added later by software; the model must render ZERO readable content.',
    'Keep the zone BLOCKS themselves (their position, size, shape and colour) — including the price callout and the contact/footer strip — just render each as an empty colour fill.',
